...
function convert() {
  ...
  const inputField = document.getElementById('floatingInputVal');
  ...
  if (rawVal.charAt(0) == '#' || (!/^2#|^8#|^10#|^16#/.test(rawVal) && !/^0b|^0x|^0o/.test(rawVal) && !/^1|^2|^3|^4|^5|^6|^7|^8|^9|^0/.test(rawVal) && rawVal != '')) {
    inputField.classList.add('is-invalid');
    inputField.setCustomValidity('Please enter prefix 2#, 8#, 10#, 16#, 0b, 0o, or 0x. No prefix defaults to decimal.');
  ...
  if(num.includes('.')){
    inputField.classList.add('is-invalid');
    inputField.setCustomValidity('Input cannot be decimal, go to floating point page (top right)');
    inputField.reportValidity();
    return;
  }
  ...
  switch (base) {
    ...
    case "0b":
      if (isBinary(num)) {
        ...
        if (num.length > 16 && homeTab.classList.contains('active')) {
          inputField.classList.add('is-invalid');
          inputField.setCustomValidity('Input cannot exceed 16 binary bits');
        } else {
          inputField.classList.remove('is-invalid');
          inputField.setCustomValidity('');
    ...
    case "0o":
      if (isOctal(num)) {
        ...
        if (decVal > 65535 && homeTab.classList.contains('active')) {
          inputField.classList.add('is-invalid');
          inputField.setCustomValidity('Input cannot exceed 16 binary bits');
        } else {
          inputField.classList.remove('is-invalid');
...
function validInput(decimalVal, base) {
  ...
  //console.log(decimalVal.toString(2).length);
...
function ascii(decimalVal) {
  ...
  for (let i = 0; i < hexStr.length; i += 2) {  // iterate over the hex string two characters at a time
    ...
    // if(parseInt(cur, 16) == 32){ Don't know what this is?
    //   return "sp";
    // }
